...
public class ListaCliente extends Lista<Cliente> {
	...
	public Cliente getByChatId(int chatId) {
		return super.lista.stream().filter(c -> c.getChatId() == chatId)
				.sorted(Collections.reverseOrder(new VerificaCliente())).findFirst().orElse(new Cliente());
	}

	public Cliente get(Cliente cliente) {
		return super.lista.stream().filter(c -> c.equals(cliente))
				.sorted(Collections.reverseOrder(new VerificaCliente())).findFirst()
