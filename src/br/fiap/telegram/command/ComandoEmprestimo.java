	...
	private void emprestar(Cliente cliente) throws Exception {
		...
		case "informar_valor": {
			...
			if (cliente.getMensagemAtual().trim().equals("")) {
				BotManager.enviarMensagem(cliente.getChatId(), "Informe o valor do empréstimo");
				return;
			} else {
				try {
		...
		case "informar_meses": {
			...
			if (cliente.getMensagemAtual().trim().equals("")) {
				BotManager.enviarMensagem(cliente.getChatId(), "Informe a quantidade de meses do empréstimo");
				return;
			} else {
				try {
					...
					if (meses > 36) {
						BotManager.enviarMensagem(cliente.getChatId(),
								"Quantidade de meses do empréstimo deve ser menor que 3 anos");
						return;
					} else {
		...
		Emprestimo emprestimo = new Emprestimo(cliente.getConta().getEmprestimo().getValorContratado(),
				cliente.getConta().getEmprestimo().getQtdeMeses(), cliente.getConta());
